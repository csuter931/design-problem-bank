...
- **Logo**: `public/dawson-logo-white.png` is the approved **reversed-colour** mark, rendered by `components/DawsonLogo.tsx` in both page headers directly on the navy — exactly what the manual specifies for blue/dark backgrounds. Keep the safety space (≈ the height of "SCHOOL") clear around it; never recolour, stretch, or crop it. **Never render it below `h-12` (48px)** — it is a stacked lockup and the "SCHOOL" line is only ~6% of its height, so smaller than that the line stops resolving and the mark reads as a smudge. Both headers use the same lockup: mark → hairline rule → app name on one line, no tagline (the mark already says "Dawson School"). The full-colour version (light backgrounds only) is parked in the gitignored `brand/` with the EPS and the manual, not in the bundle. The 💡 that remains is the "no photo" thumbnail placeholder, not branding. Brand sources (EPS, manual) live in the gitignored `brand/`. Contact `marcomm@dawsonschool.org` for brand questions.
- **Dawson Blue is a *dark* colour — never use it as text on the navy surfaces.** `text-primary` on navy measures ~1.5:1 and reads as an unreadable purple-on-purple smudge. It is a *background*: the selected/active treatment is `bg-primary text-white` (~10.6:1). Where a light accent is wanted instead, use `dawson-carolina` (~6.3:1 on navy). The same applies to `dawson-royal` and `dawson-purple`, which are surfaces, not text.
- Contrast was re-audited 2026-09-22 and the earlier "all pairs ≥ 5.5:1" claim was wrong: it missed the whole `text-primary`-on-navy family (the dashboard team pill, the selected tag chips in EditProblemModal, the wizard's current-step marker), all ~1.5:1. Fixed. White on Sea Green (2.8:1) remains unused — Sea Green stays text/border/tint only.
...
- `problems/{id}/private/detail` — the **private half** of a problem: `submitterContact` and `internalNotes`. Read: Dawson. Create: the anonymous wizard (contact only, and only while the parent is still unapproved), a Dawson user (notes only), or a super user. Update: notes by any Dawson user, anything by a super user. Delete: super users. Doc id is always `detail`. **Always delete a problem via `deleteProblemWithPrivate()`** — Firestore does not cascade to subcollections, so deleting the problem document alone strands the contact in the database.
>
